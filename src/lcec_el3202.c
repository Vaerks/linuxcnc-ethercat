...
static ec_pdo_info_t lcec_el3202_pdos_in[] = {
    ...
    {0x1A01, 11, lcec_el3202_channel2},
};
...
static const lcec_pindesc_t slave_pins[] = {
  { HAL_BIT, HAL_OUT, offsetof(lcec_el3202_chan_t ,error), "%s.%s.%s.ain-%d-error" },
  { HAL_BIT, HAL_OUT, offsetof(lcec_el3202_chan_t ,overrange), "%s.%s.%s.ain-%d-overrange" },
  { HAL_BIT, HAL_OUT, offsetof(lcec_el3202_chan_t ,underrange), "%s.%s.%s.ain-%d-underrange" },
  { HAL_S32, HAL_OUT, offsetof(lcec_el3202_chan_t ,raw_val), "%s.%s.%s.ain-%d-raw" },
  { HAL_FLOAT, HAL_OUT, offsetof(lcec_el3202_chan_t ,val), "%s.%s.%s.ain-%d-val" },
  { HAL_TYPE_UNSPECIFIED, HAL_DIR_UNSPECIFIED, -1, NULL }
};
...
static ec_pdo_info_t lcec_el3202_pdos[] = {
    {0x1a00, 9, lcec_el3202_channel1}, //* RTD TxPDO-Map Ch.1 
    {0x1a01, 9, lcec_el3202_channel2}, //* RTD TxPDO-Map Ch.2 
};
...
static ec_sync_info_t lcec_el3202_syncs[] = {
    ...
    {3, EC_DIR_INPUT, 2, lcec_el3202_pdos, EC_WD_DISABLE},
    {0xff}
};
...
int lcec_el3202_init(int comp_id, struct lcec_slave *slave, ec_pdo_entry_reg_t *pdo_entry_regs) {
  ...
  for (i=0; i<LCEC_EL3202_CHANS; i++) {
    ...
    LCEC_PDO_INIT(pdo_entry_regs, slave->index, slave->vid, slave->pid, 0x6000 + (i << 4), 0x07, &chan->error_pdo_os, &chan->error_pdo_bp);
    LCEC_PDO_INIT(pdo_entry_regs, slave->index, slave->vid, slave->pid, 0x6000 + (i << 4), 0x11, &chan->val_pdo_os, &chan->val_pdo_os);
